...
# references
c("rvest", "RSelenium", "robotstxt", "polite",  "dplyr", "xaringan") %>% 
        knitr::write_bib("scrp_workshop_files/bibliography/packages.bib")

bib_files <- c("scrp_workshop_files/bibliography/packages.bib", 
               "scrp_workshop_files/bibliography/manuals.bib")

bib_data <- lapply(bib_files, readLines)

write(unlist(bib_data),
      file = "scrp_workshop_files/bibliography/presentation.bib")
...
bibliography <- ReadBib("scrp_workshop_files/bibliography/presentation.bib", 
                        check = FALSE)
...
- `rvest` `r Citep(bibliography, "R-rvest")`, for scraping websites

--
- `RSelenium` `r Citep(bibliography, "R-RSelenium")`, for browsing the web programmatically

--
- `robotstxt` `r Citep(bibliography, "R-robotstxt")`, for checking permissions to scrape websites

--
- `polite` `r Citep(bibliography, "R-polite")`, for compliance with permissions to scrape websites
...
- Scraping static pages involves two main steps
...
```{r}
bow(url = "https://luzpar.netlify.app/exercises/exercise_6.Rhtml") %>% 
    scrape()
```
...
- Get one or more<span>*</span> HTML elements
...
<span>*</span> Note that there are two versions of the same function: singular one gets the first instance of an element, plural one gets all instances.
...
knitr::include_graphics("scrp_workshop_files/images_data/sg1.png")
```
...
Click on a menu item as we are not interested in them

.pull-left[
4. click on a menu item as we are not interested in them
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("scrp_workshop_files/images_data/sg2.png")
```
...
You can click further to exclude/include everything, or the only thing, that you want

```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("scrp_workshop_files/images_data/sg3.png")
```

---
## Static Pages &mdash; `rvest` &mdash; `html_element`

Get the link behind one element

```{r, eval=FALSE}
bow(url = "https://luzpar.netlify.app") %>%
  scrape() %>%
  html_elemen`t(c`ss = `"br+ p a+ a"`)
```

```{r, echo=FALSE}
bow(url = "https://luzpar.netlify.app") %>%
  scrape() %>%
  html_element(css = "br+ p a+ a")
```
...
- the function on this slide is the singular version
  - but the plural one would result in the same output as the selector itself is for a single element

---
## Static Pages &mdash; Finding Selectors &mdash; Chrome

To find the selector for a single item, you could also use Chrome itself
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("scrp_workshop_files/images_data/ch1.png")
```
...
bow(url = "https://luzpar.netlify.app") %>%
  scrape() %>%
  html_element(css = `"#title > div.container > div > p:nth-child(2) > a:nth-child(2)"`)
```
...
`r countdown(minutes = 2, seconds = 30, top = 0)`
...
Get the page source with the `RSelenium` package
...
Extract links with functions from both the `RSelenium` and `rvest` packages
...
26) Collect names from <https://luzpar.netlify.app/members/>

- using functions from `rvest` only

<br>

27) Repeat collecting them, this time in an automated browser

- using both `RSelenium` and `rvest`
- is the outcome different than in Exercise 26?

<br>

28) Maximize the automated browser window, and repeat again

- is the outcome different than in Exercise 26 and/or 27?
