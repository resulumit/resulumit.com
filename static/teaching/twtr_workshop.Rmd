...
- working with Twitter data
    ...
    - [an app based on Twitter data](https://resulumit.shinyapps.io/likewise/): LikeWise &mdash; a *Shiny* app that facilitates searching the tweets that you liked.
...
- Two days, on how to collect, process, and analyse data from Twitter
  ...
  - 150+ slides, 40+ exercises, and time for working on a real project
...
- Twitter provides attractive opportunities for academic research
  ...
  - at the end of 2020, a simple search for "twitter data" returns 55,000+ results on Google Scholar
...
- These restrictions vary across API types, a lot

  - e.g., [Standard v1.1](https://developer.twitter.com/en/docs/twitter-api/v1) is the most restrictive first generation APIs
      - others are [Premium v1.1](https://developer.twitter.com/en/products/twitter-api/premium-apis) and [Enterprise: Gnip 2.0](https://developer.twitter.com/en/products/twitter-api/enterprise) V1.1 APIs, with paid subscriptions, and
      - the second generation APIs, including the newly announced [Academic Research product track](https://blog.twitter.com/developer/en_us/topics/tips/2021/enabling-the-future-of-academic-research-with-the-twitter-api.html)

--
<br>

- These restrictions also vary within API types, across different operations
...
- Twitter restricts content redistribution
   ...
   - only tweet and/or user IDs can be made publicly available in datasets over 50,000 observations
      - i.e., not the tweets themselves
...
- Reproducibility of research based on Twitter data is limited in practice
  ...
  - technically, they can retrieve the same tweets with IDs
      ...
      - may even be impossible
          - e.g., tweets, or whole accounts, might be deleted in time

---
## Considerations &mdash; Changes in the Twitter APIs

- Twitter is currently switching to a new generation APIs
...
- Twitter might change the rules of the APIs game at any anytime, again
  ...
  - making the existing restrictions more or less stricter
...
- Not all changes are bad
   ...
   - among others, APIs V2 promises the [Academic Research product track](https://blog.twitter.com/developer/en_us/topics/tips/2021/enabling-the-future-of-academic-research-with-the-twitter-api.html)    
<br>
...
<br>
  - Twitter itself has no problem with it, but others might disagree
      - e.g., your country, (funding and/or research) institution, subjects
...
- Some tools of text analysis are developed for a specific language and/or context
   ...
   - e.g., dictionaries for sentiment analysis
       - might be, e.g., for English, political texts, only
   - these may not be useful, valid for different languages, and context
   
--
<br>

- Some tools of text analysis are developed for general context
...
- An unsynced directory is recommended
...
- Download the materials from <https://www.dropbox.com/s/fej5husakvnf8tz/data.zip?dl=0> 
      - to a sub-directory of your RStudio project
      - in the code that follows, my subdirectory will be called `data`
...
- Authorization to use Twitter APIs requires at least three steps
  ...
  - open a user account on Twitter
      - a personal or an institutional (perhaps, for a research project) one
      
  - with that user account, apply for a developer account
  ...
  - with that developer account, register a Twitter app
      - so that you have the keys and tokens for authorisation
      - repeated for every project

- There may be additional steps
   - e.g., registering for the [Academic Research product track](https://twitter.com/TwitterDev/status/1354143047324299264)
   - per project, depending on meeting certain criteria

---
## Twitter &mdash; Authorisation &mdash; Notes

- It is possible to interact with Twitter APIs without your own Twitter app
...
- a pre-condition for interacting with Twitter APIs
    ...
    - e.g., you must be authorized to collect tweets with R
    - and you must have an account to apply for authorisation in the first place
...
- [quanteda tutorials](https://tutorials.quanteda.io/)
   ...
   - 'a step-by-step introduction to quantitative text analysis'
...
<br>
   ...
   - may or may not be ethical and/or legal
        - while there are APIs for this task
...
---
## Data Collection &mdash; Overview &mdash; Standard v1.1 API

- We will collect data through Twitter's Standard v1.1 API    
...
- A lot has already been written on this package. See, for example, 
    ...
    - this [book](https://ggplot2.tidyverse.org/reference/) by Bob Rudis &mdash; a user of the package
...
- **`lookup_`**
    ...
    - 5 functions, for expanding an existing variable into a dataset
          - e.g., a list of IDs for tweets or users
...
- For other functions, see the [package documentation](https://cran.r-project.org/web/packages/rtweet/rtweet.pdf), including

  - those starting with `post_`, to use your account from within R
      - e.g., posting tweets, direct messages 
...
---
## Data Collection &mdash; Start Your Script &mdash; Alternatives
...
<br>
  - here we are relying on `rtweet`'s `rstats2twitter` app, which is the default
      - as `token = NULL`
...
- and assign the resulting data frame to `df_tweets`
   - so that you can observe the results better
   - hence, I recommend doing the same for all APIs searches that follow

2) Observe how the `rstats2twitter` app works
  - when you call the function, pay attention to what happens on your R console and on default browser
...
- Twitter allows user to change their usernames as well as display names
...
- The date and time data are matched to Greenwhich Mean Time
   ...
   - no matter where users actually are at the time of posting
...
- Collect the latest 100 tweets that
  ...
  - using your own keys and tokens
  
- Note that
  - this assumes you have pre-defined `tw_token`
     - with the [`create_token` function](#create-token)
     - using the keys and token that you own
...
- Collect the top 200 tweets that
  - include the .yellow-h[exact phrase]  "publish perish"
...
- Under the hood, Twitter APIs return nested lists
     - can be tidied into rectangular shape, but in itself not tidy data
...
```{r, eval=FALSE}
all_limits_now <- rate_limit(`query = "search/tweets"`)
```
...
- `retryonratelimit` is not available for all functions in the package
...
```{r, eval=FALSE}
lookup_tweets(statuses = df_ids$status_id)
```
...
```{r, eval=FALSE}
lookup_users(users  = df_usernames$screen_name)
```
...
17) Find a user ID through your browser and look them up in R

<br>

18) Look up a subset of tweets whose ids stored in `status_ids.rds` 

<br>

19) Look up a subset of users whose usernames stored in `mps.csv`

<br>

20) Check the friendship status of two MPs in the dataset
...
- This does not seem to be true
    - this code returns my home-timeline, not Wickham's
...
21) Collect the most recent tweets posted by three users 
...
22) Collect as many tweets as possible from your own home-timeline

<br>

23) Collect data from timelines of the first five MPs in `mps.csv`
...
- Get a list of followers, following .yellow-h[one] user
...
Get a list of Hadley Wickham's followers on Twitter
...
27) Collect a list of favorites by three users
- compare and contrast the resulting data with the same information available on browser

<br>

28) Collect a list of accounts retweeting a recent tweet of yours
...
29) Collect a list of places where the trends data is available
- hint: use the `trends_available` function

<br>

30) Collect the lists of trends for two locations
- compare and contrast the resulting data for two locations

<br>

31) Collect the list of trends for your location
...
32) Collect data on lists where Hadley Wickham is listed
- hint: his username is "hadleywickham"

<br>

33) For one of these lists, see who else is listed with Hadley Wickham
- compare and contrast this data with what you can see on a browser

<br>

34) Collect the latest posts from that list

<br>

35) Collect data on users subscribed to that list

<br>

36) For one of these users, see if they are subscribed to any other lists
...
```{r, eval=FALSE}
stream_tweets(q = `"UniLuzern, hslu,
                   `phluzern",`
              timeout = 30)
```
...
- the `rtweet` package does a very good job to start with
...
- In this part, please follow along the examples with actual data
   
   - you can use `tweets.rds` or your own dataset
      

---
## Data Preperation &mdash; Overview &mdash; Strings

- Most researchers would be interested in textual data on Twitter
...
- I use the `stringr` package ([Wickham, 2019](https://cran.r-project.org/web/packages/stringr/index.html)) for string operations
   ...
   - you might have a favourite already
...
- I use the `dplyr` package ([Wickham et al, 2020](https://cran.r-project.org/web/packages/dplyr/index.html) for most other data operations
...
```{r, eval=FALSE}
str_remove_all(string = tweet, pattern = "\\s([@][\\w_-]+)")
```
...
- Notice that
    ...
    - you may need multiple or complicated regular expressions
...
`r countdown(minutes = 4, seconds = 00, bottom = 0)`
...
# unlist the lists of hashtags to create strings  
  group_by(status_id) %>%
  mutate(tidy_text = str_c(unlist(hashtags), collapse = " ")) %>%
  
# split the string, create a new variable called `da_tweets`    
  unnest_tokens(output = da_hashtags, input = hashtags, token = "tweets")
```
